...
interface PhoneDao {
    ...
    @Query("SELECT * FROM Phone WHERE id IN (:ids)")
    ...
    @Query("SELECT Phone.id, Phone.phone, Phone.user_id FROM Phone INNER JOIN users ON Phone.user_id = users.id WHERE users.last_name LIKE :lastName AND users.first_name LIKE :firstName")
